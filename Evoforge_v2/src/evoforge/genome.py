...
_HUE_WORDS = [
    "Crimson", "Amber", "Gold", "Lime", "Emerald",
    "Teal", "Azure", "Indigo", "Violet", "Magenta", "Rose",
]

_CARNIVORE_NOUNS = ["Reaper", "Stalker", "Fang", "Ripper", "Hunter", "Talon"]
_HERBIVORE_NOUNS = ["Grazer", "Drifter", "Sprout", "Wanderer", "Sipper", "Bloom"]
_OMNIVORE_NOUNS = ["Forager", "Nomad", "Scavenger", "Opportunist", "Rover"]


def hue_word(h: float) -> str:
    idx = int((h % 1.0) * len(_HUE_WORDS)) % len(_HUE_WORDS)
    return _HUE_WORDS[idx]


def species_name(traits: Traits) -> str:
    """A deterministic, flavorful label derived purely from trait values.

    Same traits always produce the same name, so a lineage's name stays
    stable across frames without needing to be stored anywhere.
    """
    if traits.diet > 0.66:
        pool = _CARNIVORE_NOUNS
    elif traits.diet < 0.33:
        pool = _HERBIVORE_NOUNS
    else:
        pool = _OMNIVORE_NOUNS
    noun = pool[int(traits.radius * 37 + traits.vision_range) % len(pool)]

    adjectives = []
    if traits.max_speed > 2.2:
        adjectives.append("Swift")
    if traits.max_speed < 1.0:
        adjectives.append("Sluggish")
    if traits.radius > 7.5:
        adjectives.append("Colossal")
    if traits.radius < 4.0:
        adjectives.append("Tiny")
    if traits.aggression > 0.75:
        adjectives.append("Savage")
    if traits.vision_range > 170:
        adjectives.append("Keen-eyed")
    if traits.fertility > 1.4:
        adjectives.append("Prolific")
    if traits.efficiency > 1.15:
        adjectives.append("Efficient")
    if not adjectives:
        adjectives.append("Common")

    adj_idx = int(traits.turn_rate * 1000 + traits.metabolism * 100) % len(adjectives)
    adjective = adjectives[adj_idx]

    return f"{adjective} {hue_word(traits.color_h)} {noun}"
