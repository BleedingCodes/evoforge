...
def test_load_then_step_does_not_crash(tmp_path):
    """World.load() must restore all runtime state so step() succeeds immediately."""
    config = WorldConfig(
        width=300,
        height=200,
        initial_agents=4,
    ...
    world.save(path)
    loaded = World.load(path)
    loaded.step()  # must not raise AttributeError
    assert loaded.step_count == world.step_count + 1
